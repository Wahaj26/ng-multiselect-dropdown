...
export class MultiSelectComponent implements ControlValueAccessor {
    ...
    public selectedItems: Array<ListItem> | ListItem = [];
    ...
    onDeSelectAll: EventEmitter<Array<ListItem>> = new EventEmitter<
        Array<any>
    >();
    ...
    onItemClick($event: any, item: ListItem) {
        ...
        const allowAdd =
            ...
            (this._settings.limitSelection > 0 &&
                (this.selectedItems as Array<ListItem>).length <
    ...
    isSelected(clickedItem: ListItem) {
        let found = false;
        if (!this.isArray(this.selectedItems)) {
            if ((this.selectedItems as ListItem).id === clickedItem.id) {
                found = true;
            }
        } else {
            (this.selectedItems as Array<ListItem>).forEach(item => {
    ...
    isAllItemsSelected(): boolean {
        ...
        const itemDisabledCount = this._data.filter(item => item.isDisabled)
            .length;
        // take disabled items into consideration when checking
        return (
            this._data.length ===
            (this.selectedItems as Array<ListItem>).length + itemDisabledCount
        );
    }
    ...
    itemShowRemaining(): number {
        return (
            (this.selectedItems as Array<ListItem>).length -
            this._settings.itemsShowLimit
        );
    }

    addSelected(item: ListItem) {
        if (this._settings.singleSelection) {
            this.selectedItems = item as ListItem;
            // this.selectedItems.push(item);
        } else {
            (this.selectedItems as Array<ListItem>).push(item);
        }
    ...
    removeSelected(itemSel: ListItem) {
        if (this.defaultSettings.singleSelection) {
            (this.selectedItems as ListItem) = {} as ListItem;
        } else {
            (this.selectedItems as Array<ListItem>).forEach(item => {
                if (itemSel.id === item.id) {
                    (this.selectedItems as Array<ListItem>).splice(
                        (this.selectedItems as Array<ListItem>).indexOf(item),
                        1
                    );
                }
            });
